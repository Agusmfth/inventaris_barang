...
class UpdateFundingSourceRequest extends FormRequest
{
    ...
    {
        $source = $this->route('funding_source');
        return ['name' => ['required', 'string', 'max:255', Rule::unique('funding_sources')->ignore($source)], 'code' => ['required', 'string', 'max:20', Rule::unique('funding_sources')->ignore($source)], 'description' => ['nullable', 'string', 'max:1000'], 'is_active' => ['required', 'boolean']];
    }
