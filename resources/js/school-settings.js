document.addEventListener('DOMContentLoaded', () => {
    ...
    logo?.addEventListener('change', () => { const file=logo.files[0]; if(!file)return; const reader=new FileReader(); reader.onload=e=>{preview.innerHTML=`<img src="${e.target.result}" alt="Preview logo sekolah">`;miniLogo.innerHTML=`<img src="${e.target.result}" alt="">`;}; reader.readAsDataURL(file); });
    form?.addEventListener('submit', event => { if(form.dataset.submitting==='true'){event.preventDefault();return;} form.dataset.submitting='true';submit.disabled=true;submit.classList.add('is-loading'); });
});
